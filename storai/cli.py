...
from storai.safety import SafetyError
...
def _extract_size_gb(text: str) -> int | None:
    m = re.search(r"(\d+)\s*(?:gb|gib)", text.lower())
    if not m:
        return None
...
def _extract_mount_request(text: str) -> tuple[str | None, str, str, int | None]:
    ...
    return device, mountpoint, fstype, size_gb
...
def _handle_chat_request(planner: Planner, request: str, execute: bool, dry_run: bool) -> None:
    req_l = request.lower()
    ...
    if any(k in req_l for k in ["space", "usage", "largest", "biggest", "top", "big files", "big file", "big directories", "large files", "large file"]):
        path = "/"
    ...
    if any(k in req_l for k in ["mount", "format", "filesystem", "file system", "create filesystem"]):
        ...
        if device is None:
            ...
            try:
                device = planner.suggest_safe_disk(min_size_gb=min_size)
            except SafetyError as exc:
                console.print(f"[red]Could not auto-select a safe disk:[/red] {exc}")
                raise typer.Exit(code=2) from exc
...
def chat(
    ...
    planner = _planner(ctx.obj)
    _handle_chat_request(planner, request, execute=execute, dry_run=dry_run)


@app.command()
def agent(
    ctx: typer.Context,
    execute: Annotated[bool, typer.Option("--execute", help="Allow write execution when requested")] = False,
    dry_run: Annotated[bool, typer.Option("--dry-run/--no-dry-run", help="Print only; no execution")] = True,
) -> None:
    """Interactive agent loop for natural-language storage operations."""
    planner = _planner(ctx.obj)
    console.print("StorAI agent mode. Type 'exit' to quit.")
    while True:
        try:
            request = typer.prompt("you")
...
def plan_mount(
    ...
    size_gb: Annotated[int | None, typer.Option("--size-gb", help="Optional partition size in GiB (default: full disk)")] = None,
